  ...
  .then(function(data) {
    ...
    .then(function(data) {
      ...
      for (var i = 0; i < data.response.user.recent_brews.count; i++) {
        ...
        objectBeer = {beer: {bid: beer_bid, name: beer_name, img: beer_label, description: beer_description, brewery: beer_brewery}};
      ...
      function setSession(error) {
        ...
        User.find({ 'username': req.session.user.username }).exec( function (err, user) {
